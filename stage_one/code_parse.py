#!/usr/bin/env python3
"""
代码仓库图谱生成器 (Code Repository Graph Generator)

通过 AST / tree-sitter 静态分析解析代码仓库，输出与 datatest.graph.json 格式一致的图谱 JSON。

图谱结构：
  节点(nodes)：Repo、Package、File、TextFile、Function、Class、Attribute、Lambda
  边(edges)：contains（层级包含）、calls（函数调用）、imports（导入依赖）

支持语言：Python、JavaScript、TypeScript、Java、C、C++、C#、PHP、Kotlin

用法：
    python generate_graph.py <repo_path> [--output output.json] [--repo-name owner#repo#commit]
    python generate_graph.py <repo_path> --exclude-ext .spec.ts .test.py  # 排除特定后缀的文件
"""
...
# ---------------------------------------------------------------------------
# 排除规则：目录和文件名的 glob 模式
# ---------------------------------------------------------------------------

# 跳过以下目录（os.walk 的 dirnames 会原地修改以跳过这些目录）
...
class IDAllocator:
    def __init__(self, start: int = 10):
        self._next = start   # Repo 固定为 10，根 Package 固定为 11
...
class RepoWalker:
    ...
    def walk(self):
        """执行仓库遍历：创建 Repo 根节点 -> 遍历目录创建 Package -> 遍历文件创建 File/TextFile。"""
        # 创建根 Repo 节点
...
class TreeSitterBase:
    ...
    def _init_parser(self, grammar_package, language_func: str = "language"):
        """用给定的 tree-sitter 语法包初始化解析器。

        Args:
            grammar_package: tree_sitter_xxx 模块
            language_func:   获取 Language 对象的方法名
                            （TS 用 language_typescript，PHP 用 language_php）
        """
    ...
    def _pos(self, node) -> tuple:
        """返回节点的 (start_line, start_col, end_line)，行号从 1 开始（tree-sitter 原生从 0 开始）。"""
    ...
    def _comment(self, node) -> str:
        """提取函数/类 body 的首条注释/docstring。若无则返回 "null" 字符串。"""
    ...
    def _create_func(self, name: str, node, header_node=None, parent_id=None) -> int:
        """创建 Function 节点并添加 contains 边，返回新节点 ID。"""
    ...
    def _create_class(self, name: str, node, parent_id=None) -> int:
        """创建 Class 节点并添加 contains 边，返回新节点 ID。"""
        sl, sc, el = self._pos(node)
        text = self._node_text(node)
    ...
    def _create_attr(self, name: str, node, parent_id=None, attr_type="null") -> int:
        """创建 Attribute 节点并添加 contains 边，返回新节点 ID。"""
        sl, sc, el = self._pos(node)
        text = self._node_text(node)
    ...
    def _create_lambda(self, text: str, node, parent_id=None) -> int:
        """创建 Lambda 节点并添加 contains 边，返回新节点 ID。"""
        sl, sc, el = self._pos(node)
    ...
    def _record_call(self, caller_id: int, callee_name: str, line: int):
        """记录一条待解析的函数调用（Phase 3 中通过符号表解析目标 ID）。"""
        if callee_name:
    ...
    def _record_import(self, name: str, module=None):
        """记录一条待解析的导入声明（Phase 3 中通过符号表解析目标 ID）。"""
        if name:
...
class PythonFileAnalyzer(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node: ast.ClassDef):
        """访问类定义：创建 Class 节点，注册符号表，递归遍历类体。"""
    ...
    def _process_function(self, node: "Union[ast.FunctionDef, ast.AsyncFunctionDef]"):
        """处理函数/方法定义：创建 Function 节点，确定父作用域（File/Class/嵌套函数）。"""
        docstring = ast.get_docstring(node)
        comment = docstring if docstring else "null"
    ...
    def visit_AnnAssign(self, node: ast.AnnAssign):
        """访问类型注解赋值（如 x: int = 5）：仅在模块/类级别提取 Attribute 节点。"""
        if self.current_function_id is not None:
            self.generic_visit(node)
            return
    ...
    def visit_Lambda(self, node: ast.Lambda):
        """访问 Lambda 表达式：创建 Lambda 节点，父节点为最近的 Function/Class/File。"""
        lam_id = self.id_alloc.allocate()
        end_loc = node.end_lineno or node.lineno
        text = self._extract_text(node.lineno, end_loc) if hasattr(node, 'lineno') else ""
    ...
    def visit_Call(self, node: ast.Call):
        """访问函数调用：提取被调用名称，记录到 pending_calls 待后续跨文件解析。"""
    ...
    def visit_Import(self, node: ast.Import):
        """访问 import X 声明：记录到 pending_imports。"""
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        """访问 from X import Y 声明：跳过相对导入，记录到 pending_imports。"""
                ...
                for decl in self._children_by_type(node, "variable_declarator"):
                    ...
                    if name_node:
                        ...
                        if val:
                            ...
                            attr_id = self._create_attr(self._node_text(name_node), decl, parent_id=pid)
                            if parent_id == self.file_id:
...
class CppAnalyzer(CAnalyzer):
    """C++ 代码分析器。继承 CAnalyzer，额外解析 class/struct、成员变量。"""
    def analyze(self):
        try:
        ...
        if t == "class_declaration":
            ...
            for decl in self._children_by_type(node, "variable_declarator"):
                ...
                if name_node:
                    ...
                    atype = self._node_text(type_node) if type_node else "null"
                    attr_id = self._create_attr(self._node_text(name_node), decl, pid, attr_type=atype)
                    if pid == self.file_id:
                        self.graph.add_symbol(self._node_text(name_node), attr_id)
            ...
            name_node = self._child_by_type(node, "identifier")
            if name_node:
                attr_id = self._create_attr(self._node_text(name_node), node, pid)
                if pid == self.file_id:
                    self.graph.add_symbol(self._node_text(name_node), attr_id)
    ...
    def _extract_all(self, node, parent_id, current_func_id):
        ...
        if t == "class_declaration":
            ...
            if name_node:
                ...
                atype = self._node_text(type_node) if type_node else "null"
                attr_id = self._create_attr(self._node_text(name_node), node, pid, attr_type=atype)
                if pid == self.file_id:
                    self.graph.add_symbol(self._node_text(name_node), attr_id)
...
def resolve_imports(graph: GraphOutput):
    """解析 pending_imports：在全局符号表中查找被导入名称 -> 创建 imports 边。"""
    resolved = 0
...
def detect_repo_name(repo_path: str) -> str:
    """自动检测仓库的 owner#repo#commit 格式名称（从 git remote + HEAD 获取）。"""
    owner = "unknown"
...
def main():
    ...
    # Phase 1: 遍历仓库目录树，创建 Package/File/TextFile 节点
    logger.info("Phase 1: Walking repository tree...")
    t0 = time.time()
    walker = RepoWalker(repo_path, repo_name, id_alloc, graph, exclude_extensions)
    walker.walk()
    logger.info(f"  Phase 1 done in {time.time() - t0:.2f}s")

    # Phase 2: 按语言分组，逐文件解析（AST / tree-sitter）
    files_by_lang = {}
    ...
    logger.info(f"  Parsed {total_parsed} files ({total_failed} failed) in {time.time() - t0:.2f}s")

    # Phase 3: 跨文件解析 —— 通过全局符号表解析调用和导入关系
    ...
    logger.info(f"  Phase 3 done in {time.time() - t0:.2f}s")

    # Phase 4: 序列化输出
    logger.info("Phase 4: Writing output...")
    t0 = time.time()
    graph.to_json(args.output)
    logger.info(f"  Phase 4 done in {time.time() - t0:.2f}s")
